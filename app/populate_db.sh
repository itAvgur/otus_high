#!/bin/sh
   ...
   docker exec -it otus_pg psql otus_high -c "ALTER SEQUENCE friends_id_seq restart with 2000;" &&
   docker exec -it otus_pg psql otus_high -c "COPY posts FROM '/data/posts.csv' DELIMITER ',' CSV HEADER;" &&
   docker exec -it otus_pg psql otus_high -c "ALTER SEQUENCE posts_id_seq restart with 2000;" &&
